...
class SteemSignature extends SteemKey {
  ...
  bool verify(String data, SteemPublicKey publicKey) {
    var d = sha256.convert(utf8.encode(data));
  ...
  static int calcPubKeyRecoveryParam(
    ...
    for (var i = 0; i < 4; i++) {
      var Qprime = recoverPubKey(e, ecSig, i);
  ...
  static ECPoint? recoverPubKey(BigInt e, ECSignature ecSig, int i) {
    ...
    // 1.1 Let x = r + jn
    var x = isSecondKey > 0 ? r + n : r;
    var R = SteemKey.secp256k1.curve.decompressPoint(isYOdd, x);
    var nR = (R * n)!;
    if (!nR.isInfinity) {
    ...
    var Q = multiplyTwo(R, s, G, eNeg)! * rInv;
    return Q;
  }
